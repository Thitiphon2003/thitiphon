...
if(isset($_GET['delete'])) {
    ...
    $_SESSION['success'] = 'ลบร้านค้าเรียบร้อยแล้ว';
    header('Location: admin_sellers.php');
    exit();
}

// จัดการการเพิ่ม/แก้ไขร้านค้า
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['add_seller'])) {
        ...
        $sql = "INSERT INTO sellers (name, phone, email, owner_id, status, created_at) VALUES (?, ?, ?, ?, ?, NOW())";
        query($sql, [$name, $phone, $email, $owner_id, $status]);
        $_SESSION['success'] = 'เพิ่มร้านค้าเรียบร้อยแล้ว';
        header('Location: admin_sellers.php');
        exit();
    }
}
...
// ดึงข้อมูลร้านค้า
$sellers = fetchAll("SELECT s.*, u.username, u.firstname, u.lastname 
                     FROM sellers s 
                     LEFT JOIN users u ON s.owner_id = u.id 
                     ORDER BY s.id");

// ดึงข้อมูลผู้ใช้สำหรับเลือกเจ้าของร้าน
$users = fetchAll("SELECT id, username, firstname, lastname FROM users WHERE status = 'active'");
?>
...
<head>
    ...
    <title>จัดการร้านค้า - SHOP.COM</title>
...
<body>
    <div class="admin-container">
        ...
        <div class="sidebar">
            ...
            <div class="nav-menu">
                ...
                <a href="admin_sellers.php" class="nav-item active"><i class="fas fa-store"></i> จัดการร้านค้า</a>
        ...
        <div class="main-content">
            <div class="top-bar">
                <h1><i class="fas fa-store"></i> จัดการร้านค้า</h1>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($seller['owner_id']): ?>
                                    <?php echo $seller['firstname'] . ' ' . $seller['lastname']; ?>
                                <?php else: ?>
                                    -
                                <?php endif; ?>
                            </td>
                            <td><?php echo number_format($seller['total_sales']); ?></td>
                            <td><?php echo $seller['status']; ?></td>
                            <td>
                                <button class="action-btn btn-warning" onclick="editSeller(<?php echo $seller['id']; ?>)">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="action-btn btn-danger" onclick="deleteSeller(<?php echo $seller['id']; ?>)">
    ...
    <div id="addModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>เจ้าของร้าน (ผู้ใช้)</label>
                ...
                <div class="form-actions">
                    <button type="submit" name="add_seller" class="btn-success">บันทึก</button>
    ...
    <script>
        ...
        function editSeller(id) {
            window.location.href = 'admin_sellers.php?edit=' + id;
        }
        
        function deleteSeller(id) {
            if(confirm('คุณแน่ใจหรือไม่ที่จะลบร้านค้านี้?')) {
                window.location.href = 'admin_sellers.php?delete=' + id;
            }
        }
    </script>
</body>
